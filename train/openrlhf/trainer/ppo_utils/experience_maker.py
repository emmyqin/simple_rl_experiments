...
class Samples:
    ...
    reward: (B,) Optional list of rewards.
    """
...
class NaiveExperienceMaker(ABC):
    ...
    def make_experience_list(self, all_prompts: Union[str, List[str]], **generate_kwargs) -> List[Experience]:
        ...
        """
        print(f"----- Naive experience_list 875: ALL EXAMPLES-----{type(all_prompts)}")
...
class RemoteExperienceMakerEnvMultiturn(NaiveExperienceMaker):
    ...
    def make_experience_list(self, all_prompts: Union[str, List[str]], **generate_kwargs) -> List[Experience]:
        ...
        print(f"------make_experience_list 3361: ALL EXAMPLES-----{type(all_prompts)}")
    ...
    def generate_samples(self, all_prompts: List[str], **generate_kwargs) -> List[Samples]:
        ...
        """
        print(f"------generate_samples 3378: ALL EXAMPLES-----{type(all_prompts)}")
    ...
    def _generate_vllm(self, all_examples: List[dict], **kwargs) -> List[Samples]:
        ...
        print(f"------ALL EXAMPLES-----{type(all_examples)}")
        ...
        for i, llm in enumerate(llms):
            ...
            if prompt_token_ids:
                print(f"------------WHAT IS THE ENV MAKER HERE: {vars(self.strategy.args).get('env_file', False)}")
                datum = all_full_data[i * batch_size : (i + 1) * batch_size]
                all_output_refs.append(
                    llm.generate.remote(sampling_params=sampling_params, 
                                        multiturn=True, 
                                        full_data=datum,
                                        env_maker=self.strategy.args.env_maker)
                )
        ...
        for i in range(0, len(all_outputs), args.micro_rollout_batch_size):
            ...
            if not self.packing_samples:
                ...
                for conversation, reward in outputs:
                    ...
                    output_len = len(conversation.all_output_tokens[input_len:]) 
                    output_ids = list(conversation.all_output_tokens)[input_len:] + [pad_token_id] * (max_output_len - output_len)
